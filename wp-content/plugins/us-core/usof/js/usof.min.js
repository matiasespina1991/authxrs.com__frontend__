...
$usof.genUniqueId=function(){return Math.random().toString(36).substr(2,9)};$usof.mixins.Events={on:function(eventType,handler){if(this.$$events===undefined){this.$$events={}}
...
this.fields[fieldName].on('change',function(field){this.updateVisibility(field.name)}.bind(this));if(!!this.isGroupParams){this.updateVisibility(fieldName,!1,this.getCurrentShown(fieldName))}}},updateVisibility:function(fieldName,isAnimated,isCurrentShown){if(!fieldName||!this.showIfDeps[fieldName])return;if(isAnimated===undefined){isAnimated=!0}
...
return values},setValues:function(values,quiet){for(var fieldId in values){if(!values.hasOwnProperty(fieldId)||this.fields[fieldId]==undefined){continue}
var currentValue=values[fieldId];this.setValue(fieldId,currentValue,quiet);if(!quiet){this.fields[fieldId].trigger('change',[currentValue])}}
...
var value=values.join(this._separator);this.$input.val(value);this.trigger('change',[value])},getValue:function(){return this.$input.val()},setValue:function(value,quiet){var values=$.isArray(value)?value:(''+value).split(this._separator);this.$checkboxes.each(function(){$(this).removeAttr('checked').prop('checked',$.inArray(this.value,values)>-1)});this.$input.val($.isArray(value)?value.join(this._separator):value);if(!quiet){this.trigger('change',[value])}}};$usof.field.check_table={getValue:function(){var value={};$.each(this.$input,function(){value[this.value]=(this.checked)?1:0});return value},setValue:function(value,quiet){$.each(this.$input,function(){$(this).attr('checked',(value[this.value]===undefined||value[this.value]==1)?'checked':!1)})}}}(jQuery);!function($,undefined){var _window=window,_document=document;if(_window.$usof===undefined){return}
...
this.$input.on('change',this._events.changeSelect.bind(this));this.$container.attr('selected-value',this.$input.val());this._toggleGridLayoutDesc.call(this)},_events:{changeSelect:function(){var value=this.$input.val();this.$container.attr('selected-value',value);if(!this.hintsJson.no_posts){if(value.length&&value.match(/\d+/)){var hint='';if(this.hintsJson.hasOwnProperty('edit_url')){var regex=/(<a [^{]+)({{post_id}})([^{]+)({{hint}})([^>]+>)/;hint=this.hintsJson.edit_url.replace(regex,'$1'+value+'$3'+this.hintsJson.edit+'$5')}
this.$hint.html(hint)}else{this.$hint.html('')}}
this._toggleGridLayoutDesc.call(this)}},_toggleGridLayoutDesc:function(){if(!this.$row.hasClass('for_grid_layouts')){return}
var value=this.getValue(),isVC=this.$row.hasClass('us_select_for_vc'),$addDesc=$('.us-grid-layout-desc-add',isVC?this.$row.parent():this.$row),$editLink=$('.us-grid-layout-desc-edit',isVC?this.$row.parent():this.$row);if($.isNumeric(value)){$('.edit-link',$editLink).attr('href',(this.$container.data('edit_link')||'').replace('%d',value));$addDesc.addClass('hidden');$editLink.removeClass('hidden')}else{$addDesc.removeClass('hidden');$editLink.addClass('hidden')}}}}(jQuery);!function($,undefined){var _window=window,_document=document;if(_window.$usof===undefined){return}
...
return '<span class="usof-autocomplete-selected" data-key="'+key+'">\
			...
			</span>'},getValue:function(){return this.$value.val()},setValue:function(value,quiet){this.$value.val(value);this._initValues.call(this);if(!quiet){this.trigger('change',[value])}}}}(jQuery);!function($,undefined){var _window=window;if(_window.$usof===undefined){return}
...
return value},setValue:function(value,quiet){if(!this.inited)return;var newValue={};if(this.format==='serialized'&&((''+value).substr(0,4)==='url:'||(''+value).indexOf('|')!==-1)){var pairs=value.trim().split('|');for(var i=0;i<pairs.length;i ++){var param=pairs[i].split(':');if(param[0]&&param[1]){newValue[param[0]]=usof_rawurldecode(param[1])}}}else if(value&&this.format==='jsons'){newValue=JSON.parse(value)}else{newValue=$.extend({},value||{})}
if(!!newValue.url){newValue.url=usof_rawurldecode(newValue.url)}
this.$url.val(newValue.url);this.$target.attr('checked',(newValue.target==='_blank')?'checked':!1);if(typeof value!=='string'){value=JSON.stringify(value)}
