 ...
 */
 // TODO: The rename to `usofMod` because there is `usMod` in 'common/js/base/_general.js'
...
! function( $, undefined ) {
	...
	$usof.field[ 'checkboxes' ] = {
		...
		setValue: function( value, quiet ) {
			// For support for array of values separated by commas
			if ( value.indexOf( ',' ) !== -1 ) {
				value = value.split( ',' );
			}
			$.each( this.$input, function() {
	...
	$usof.field[ 'select' ] = {
		init: function( options ) {
			...
			this.$hint = $( '.usof-form-row-hint-text', this.$container );
			this.$hintsJson = $( '.usof-form-row-hint-json', this.$container );
	...
	$usof.field[ 'imgradio' ] = {
		init: function() {
			// Elements
			this.$container = $( '.usof-imgradio', this.$row );
			this.$inputs = $( 'input[type="radio"]', this.$container );

			// Events
			this.$container
				.on( 'change', 'input[type="radio"]', this._events.changeItem.bind( this ) );
		},
		/**
		 * Event handlers
		 * @private
		...
		_events: {
			...
			changeItem: function( e ) {
				this.setValue( e.currentTarget.value, true );
			}
		},
		/**
		 * Sets the value.
	...
	$usof.field[ 'editor' ] = {
		...
		init: function() {
			...
			this.$template = $( 'script[type="template/html"]', this.$container );

			// Add editor to container
			this.$template
				.before( this.$template.html() );

			// Get textarea
			this.$textarea = $( 'textarea', this.$container );

			// Variables
			this.editorId = this.$textarea.attr( 'id' );
		...
		initEditor: function() {
			...
			// Remove old editor
			_window.wp.editor.remove( this.editorId );
			...
			var pid = setTimeout( function() {
				_window.wp.editor.initialize( this.editorId, _window.tinyMCEPreInit.mceInit[ this.editorId ] || {} );
				_window.quicktags( _window.tinyMCEPreInit.qtInit[ this.editorId ] || {} );
				// The open tinymce by default
		 ...
		 *
		 * @private
		 * @param {object} e TinyMCE Event
		 */
		_changeTinymceContent: function( e ) {
			var value = _window.tinymce.get( this.editorId ).getContent();
			if ( value.trim() == this.getValue().trim() ) {
				return;
			}
	...
	$usof.field[ 'design_options' ] = {
		init: function() {
				...
				$( '[data-responsive-state-content="default"]', this.$container )
					.each( function( _, content ) {
						...
						this.extStates.map( function( responsiveState ) {
							$content.after( $content.clone().attr( 'data-responsive-state-content', responsiveState ).addClass( 'hidden' ) );
						}.bind( this ) );
					}.bind( this ) );
			}
			...
			this.$container.find( '.us-builder-states' ).toggleClass( 'hidden', ! this.extStates.length );
		...
		_events: {
			...
			changeValue: function() {
				...
				resultValue = ( JSON.stringify( resultValue ) !== '{}' )
					// Due to the nature of WPBakery Page Builder, we convert special characters standard usof_rawurlencode
					// function
			...
			toggleResponsive: function( e ) {
				...
				if ( this.defaultGroupValues.hasOwnProperty( groupKey ) ) {
					if ( !! isEnaled ) {
						this.$container
							.find( '[data-accordion-content="' + groupKey + '"] > .us-builder-states > [data-responsive-state="default"]' )
							.trigger( 'click' );
					}
					this.extStates.map( function( responsiveState ) {
						...
						if ( this.groupParams.hasOwnProperty( responsiveState ) && this.groupParams[ responsiveState ] instanceof $usof.GroupParams ) {
							this.groupParams[ responsiveState ].setValues( values );
						}
						// Checking and duplicating wiretap related fields
						if ( ! isEnaled && this.groupParams.hasOwnProperty( responsiveState ) ) {
			...
			changeResponsiveStates: function( e ) {
				...
				$target
					...
					.addClass( 'hidden' )
					.filter( '[data-responsive-state-content="' + $target.data( 'responsive-state' ) + '"]' )
					.removeClass( 'hidden' );
			}
		},
		/**
		...
		getValue: function() {
			...
			return this.hasStringValue ? value : JSON.parse( usof_rawurldecode( value ) || '{}' );
		},
		/**
		 * Set the value.
		 *
		...
		setValue: function( value ) {
			...
			var savedValues = {};
			if ( this.hasStringValue && typeof value === 'string' ) {
				try {
			...
			var pid = setTimeout( function() {
				...
				$( '[data-responsive-state="default"]', this.$container )
					.trigger( 'click' );
			...
			// Set value
			this.$input.val( this.hasStringValue ? value : usof_rawurlencode( JSON.stringify( value ) ) );
	...
	$usof.Group.prototype = {
		...
		init: function( elm, options ) {
			...
			this.$field
				.on( 'click', '.usof-icon-duplicate', this.duplicateGroup.bind( this ) )
				.on( 'click', '.usof-form-group-item-controls > .usof-icon-delete', function( event ) {
					event.stopPropagation();
			...
			if ( this.isSortable ) {
				...
				this.$field
					...
					.on( 'mousedown', '.usof-icon-move', this._dragStart.bind( this ) );
				// Event handlers
				this._events = {
					_maybeDragMove: this._maybeDragMove.bind( this ),
					_dragMove: this._dragMove.bind( this ),
