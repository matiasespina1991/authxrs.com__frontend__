...
// TODO: Check relevance and possibly delete the function
...
if ( ! function_exists( 'us_implode_atts' ) ) {
	 ...
	 * @param bool $is_shortcode Attributes for the shortcode
	 * @param string $separator Separator between parameters
	 * @return string
	 */
	function us_implode_atts( $params = array(), $is_shortcode = FALSE, $separator = ' ' ) {
		if ( empty( $params ) OR ! is_array( $params ) ) return '';
		...
		if ( ! $is_shortcode AND ! empty( $params['href'] ) ) {
			$params['href'] = rawurldecode( (string) $params['href'] );
		}

		$result = array();
		foreach ( $params as $key => $value ) {
			if ( ! $is_shortcode AND $value === '' ) {
				if ( ! in_array( $key, $not_empty_atts ) ) {
					$result[] = esc_attr( $key );
				}
			} else {
