...
foreach ( $markers as $index => $marker ) {
	...
	if ( ! empty( $marker['marker_address'] ) ) {
		$script_options['markers'][] = array(
			...
			'marker_img' => ( ! empty( $marker['marker_img'] ) ) ? wp_get_attachment_image_src( (int) $marker['marker_img'], 'thumbnail' ) : NULL,
...
if ( $custom_marker_img != '' AND $custom_marker_img_src = wp_get_attachment_image_url( $custom_marker_img, 'thumbnail' ) ) {
	$custom_marker_size = (int) $custom_marker_size;
...
$script_options = apply_filters( 'us_maps_js_options', $script_options, get_the_ID(), $_atts['id'] );
