...
! function( $, undefined ) {
	...
	$usof.field[ 'header_builder' ] = {
		...
		init: function( options ) {
			...
			$( 'label', this.sideOptions.fields.orientation.$row ).on( 'click', function( event ) {
			...
			this.$stateTabs.on( 'click', function( event ) {
				var $stateTab = $( event.target ),
					newState = $stateTab.usMod( 'icon_devices' );
		...
		setState: function( newState, force ) {
			...
			this.$stateTabs.removeClass( 'active' ).filter( '.icon_devices_' + newState ).addClass( 'active' );
